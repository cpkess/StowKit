...
3. In **Inbox**, a review card takes you through documents one at a time: **Use Suggestions & Next** (⌘↩), a button per collection to file it, or **Mark Reviewed & Next** (⌥⌘↩); ⌘[ and ⌘] move between documents. **Organize Inbox…** (⇧⌘I) handles the whole Inbox at once: Suggest for All, adjust any collection, then apply. Confident suggestions are filed automatically; fairly sure ones are filed and marked **Filed automatically** in the list; uncertain ones stay in Inbox marked **Needs review**. **Review Suggestions** shows what was suggested and why. Choose **Mark Reviewed** when a document is organized. A document whose text can't be read is never removed; you can still preview and edit it, and it stays in Inbox until the text problem is resolved.
...
0. **Overview** (sidebar, ⇧⌘H) is where StowKit opens: a search field over the whole archive, what needs you (Inbox, overdue, due or expiring soon, not in a collection), what arrived recently, and your collections with counts. Each count opens exactly the documents behind it.
5. Search from Overview, or with **⌘F** in any view. Inside the Inbox, Favorites, or a collection, search covers that place and a control above the list offers **Everywhere**; **⇧⌘F** widens it from anywhere, and a search with no local results offers the archive-wide count in one click. Widening changes the place only — filter chips still apply, and Trash is searched only from Trash. Search ranks titles, senders, metadata, and document text together, including words found on different pages. Type word prefixes such as `refrig warranty`, or use quotes for an exact phrase such as `"renewal date"`. Matches ignore case and accents and appear in highlighted snippets. Results load 50 at a time; use **Load More** to continue. Search results use relevance order; the Sort menu controls browsing without a query.
5d. **Upcoming** (sidebar) lists documents overdue, or due or expiring within 90 days. A document with a due or expiry date offers **Add Reminder**, which adds it to Apple Reminders only when you click.
5c. **People & Things** (sidebar) lists the people, organizations, and things your documents mention; give each a kind, rename it everywhere, or click it to see its documents. A document's **Related** list shows others that mention the same person or thing, such as a refrigerator's receipt, warranty, and manual.
5b. Select several documents with ⌘- or ⇧-click to change them together: collections, tags, sender, type, date, review state, favorites, Suggest Again, or Trash.
5a. Use **Filter** under the search field to narrow any view by tag, sender, type, date, or due and expiry dates, and **Save View…** to keep it in the sidebar. The sidebar's **Tags** section lists your most used tags; right-click one to rename or remove it everywhere.
...
12. **Import from paperless-ngx…** (File menu) brings in a folder written by paperless-ngx's document exporter: originals, titles, dates, senders, types, tags, notes, and amount and due-date custom fields, with paperless's text kept. Your paperless details are protected from suggestions. Built from paperless-ngx's documented format and not yet tried on a real export.
...
**Export Archive…** (File menu, ⇧⌘E) writes every document outside Trash, its text, and a JSON/CSV manifest to a folder you choose, with each copy verified against its original's SHA-256; it is readable without StowKit. iCloud is sync, not backup: a deletion syncs too. **The default build is a local archive, not a cloud backup.** A provisioned build can opt into iCloud in Settings; see the [iCloud setup and acceptance guide](docs/ICLOUD_SETUP.md). PDF parsing, Vision OCR, document understanding, and classification run locally, with no external AI service. Back up the complete archive directory while the app is closed, including database sidecars and originals. No telemetry or external AI is included. Opting into iCloud uploads originals, metadata, and extracted text through CloudKit.
...
[v1.6.1](https://github.com/cpkess/StowKit/releases/tag/v1.6.1) is a Gamergrams Developer ID signed and Apple-notarized DMG for macOS 14 or later, with Apple Silicon and Intel binaries, using Production CloudKit, and includes the Share → StowKit extension. It updates itself from these GitHub releases. Read the [release notes](docs/releases/v1.6.1.md) first: it has been tested on one Mac only, several 1.5 features have not yet been used on a real archive, a Mac on 1.4 or earlier stalls on 1.5's document fields until it updates, and household sharing is experimental and untested across accounts. Earlier releases: [v1.6.0](docs/releases/v1.6.0.md), [v1.5.0](docs/releases/v1.5.0.md), [v1.4.0](docs/releases/v1.4.0.md), [v1.3.0](docs/releases/v1.3.0.md), [v1.2.0](docs/releases/v1.2.0.md), [v1.1.0](docs/releases/v1.1.0.md), [v1.0.0](docs/releases/v1.0.0.md), [v0.6.0-alpha.2](docs/releases/v0.6.0-alpha.2.md), and a device-restricted alpha.1.

To package an already signed build without changing its signature:

```sh
scripts/package-dmg.sh /path/to/StowKit.app 1.6.1 build/releases
```
