...
actor OCRService: DocumentTextExtractor {
    ...
    func extractPage(for input: ExtractionInput, index: Int) throws -> ExtractedPage {
        try autoreleasepool { try extract(input, index: index) }
    }
    private func extract(_ input: ExtractionInput, index: Int) throws -> ExtractedPage {
        try Task.checkCancellation()
        if input.isImage {
        ...
        let image = try Self.render(cgPage)
    ...
    private func recognize(_ image: CGImage) throws -> ExtractedPage {
        try Task.checkCancellation()
        ...
        return ExtractedPage(text: TextNormalization.normalize(text), method: .ocr)
    }
