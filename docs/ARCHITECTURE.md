...
The first local foundation is implemented in additive schema V5. Imports, manual edits, Trash/Restore, automatic filing, explicit suggestion acceptance, and custom collection creation save sync snapshots in the same transaction as their existing metadata/search work. A journal encoding/fetch error rolls back the associated mutation. V1–V4 model definitions remain unchanged. Existing documents are not scanned or enqueued on launch; the explicit activation backfill described below establishes the initial local snapshots before future cloud use.

`SyncRecord` retains one latest snapshot per document/collection. Each changed snapshot receives a new operation UUID; unchanged fields retain their own operation stamp and manual protection. Repeated offline edits coalesce instead of growing an unlimited edit log. Acknowledgments match the exact operation UUID, so an older delivery cannot clear a newer pending edit. Acknowledged snapshots remain as local field-provenance state; they are not server baselines. A stable collection UUID bridge and explicit false membership fields preserve identities and removed memberships in snapshots. The UI and frozen document model still use collection names; full normalized collection storage and alias reconciliation are unfinished.

`LocalSyncDriver` sends one bounded batch through an injected protocol, restricts acknowledgments to that batch, and retains unacknowledged work on failure. Only tests construct a fake transport. A separate local receiver exercises incoming pages; neither component is a CloudKit adapter. `SyncMergePolicy` tests three-way independent-field merging, sticky manual protection (including empty values), preserved alternatives for concurrent manual conflicts, deterministic automatic choices, membership removals, and Trash/Restore. V6 now persists the results as described below; conflict presentation in the application remains future work.

Original consumers now call the storage actor's `localOriginal(for:)`: PDF/image previews, thumbnail generation, Quick Look, Open Copy, and OCR. It checks local presence, regular-file status, and expected size. Imports still establish hash integrity; this access check does not rehash every preview or detect same-size external corruption. Missing files produce an actionable error. No download, lease, or eviction implementation is implied by this boundary.

Additive schema V6 stores backfill checkpoints, incoming-feed progress, server baselines, and conflict history. `backfillSyncBatch` explicitly processes up to 256 records per call, first collection definitions and then documents ordered by immutable content hash. Each page and its cursor commit together; existing journal snapshots are retained, and new imports already journal themselves even if their hash falls behind the cursor. Repeated calls after completion do no scan. Legacy documents without analysis provenance receive conservative manual protections in their sync snapshots. This activation API is not invoked by normal app startup.

`LocalSyncReceiver` fetches one page from an injected transport using the persisted opaque cursor. `applyIncomingPage` accepts at most 64 complete metadata records for existing documents in this archive. Format, field types, immutable identity, payload size, and known collection membership are validated. Unsupported new document/collection identities stop the page without advancing its cursor; asset-backed document creation and normalized collection import are not implemented. The full page saves materialized metadata, processing/protection changes, search receipts, server baselines, conflicts, and cursor in one transaction. A failed page rolls back every change; replayed or out-of-order pages are rejected so the caller must refetch from current progress. There is no assumption that CloudKit tokens can be numerically compared.

Conflicting manual values retain both alternatives in durable records. The server value is displayed while the conflict is open, and the document is excluded from outgoing batches. Unrelated local edits remain pending. A later remote edit preserves the unresolved local alternative in a new conflict; prior alternatives remain in history. Explicit repository resolution selects either saved value, marks it manual, generates a fresh outgoing operation, and updates the search journal atomically. Stale resolutions fail. An ordinary local edit to the conflicted field supersedes the conflict while preserving history; remote agreement with the local alternative also resolves it. None of these conflict APIs are exposed in the UI yet.

Next: finish normalized collection/duplicate-alias migration and new remote document handling, then expose conflict review and activation in the UI. The outbound fake protocol still lacks conditional server saves and account/zone binding, so it must not be connected to CloudKit as-is. Live CloudKit and cache eviction follow the separate acceptance gates in the note.
