...
`LocalSyncDriver` sends one bounded batch through an injected protocol, restricts acknowledgments to that batch, and retains unacknowledged work on failure. Only tests construct a fake transport. This is an outbound receipt foundation, not a CloudKit adapter or a bidirectional replication engine. `SyncMergePolicy` tests three-way independent-field merging, sticky manual protection (including empty values), preserved alternatives for concurrent manual conflicts, deterministic automatic choices, membership removals, and Trash/Restore. Returned conflicts are values only; server baselines, durable conflict storage/resolution, received-record validation, and transactional incoming application remain required before live sync.
...
Next: implement resumable activation backfill and server-baseline/incoming-conflict persistence against the fake transport, then finish normalized collection and duplicate-alias migration. Live CloudKit and cache eviction follow the separate acceptance gates in the note.
